...
namespace TelebotStandBy.Service.RobotImpleFunc
{
    public class RobotUrlMethod : ApiMethod, IPayUrl
    {
        ...
        public RobotUrlMethod(IRobotApi robotApi) : base(robotApi) { }
        public void QueryPayUrl(int userid, string payUrl)
        {
            
            try
            {
                string Url = PreQueryPayUrl(payUrl);
                DataTable dt = Verify.GetPaymentUrl(Url);
            ...
            {
                _robotApi.RobotApi(000, $"error occur when {userid} query url err: {ex.Message}");
                _robotApi.RobotApi(userid, "後台無此接口");
            }
        ...
        public static string PreQueryPayUrl(string Url)
        {
            return Url;
        }
